...
try {
    if (isset($_GET['paypal']) && !$_GET['paypal'] == "") {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $conn->query("INSERT INTO `mythicaldash_payments` (`code`, `coins`, `ownerkey`, `getaway`, `status`) VALUES ('" . mysqli_real_escape_string($conn, $payment_id) . "', '" . mysqli_real_escape_string($conn, '7') . "', '" . mysqli_real_escape_string($conn, $_COOKIE['token']) . "', 'paypal', 'paid');");
